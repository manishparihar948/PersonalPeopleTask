...
import Foundation
import XCTest
// First import main project file so we can access other file from the main file
@testable import PersonalPeopleAPI

// Initially we want to make every test fails and then we write in reverse compatibility
class JSONMapperTests: XCTestCase {
    ...
    func test_with_valid_json_successfully_decodes() {
        ...
        // Check all the object is valid or not?
        XCTAssertEqual(userResponse?.page,1, "Page number should be 1")
        XCTAssertEqual(userResponse?.perPage,6, "Page item should be 6")
        XCTAssertEqual(userResponse?.total, 12, "Total items should be 12")
        XCTAssertEqual(userResponse?.totalPages, 2, "Total pages should be 2")
        
        // Check the object in Dictionay is valid or not?
        XCTAssertEqual(userResponse?.data.count,6, "The Total number of user should be 6")
        
        // First object of an Array
        ...
        XCTAssertEqual(userResponse?.data[0].avatar,"https://reqres.in/img/faces/1-image.jpg", "The avatar should be https://reqres.in/img/faces/1-image.jpg")
        
        // Second object of an Array
        ...
        XCTAssertEqual(userResponse?.data[1].avatar,"https://reqres.in/img/faces/2-image.jpg", "The avatar should be https://reqres.in/img/faces/2-image.jpg")
        
        // Third object of an Array
        ...
        XCTAssertEqual(userResponse?.data[2].avatar,"https://reqres.in/img/faces/3-image.jpg", "The avatar should be https://reqres.in/img/faces/3-image.jpg")
        ...
        XCTAssertEqual(userResponse?.data[3].avatar,"https://reqres.in/img/faces/4-image.jpg", "The avatar should be https://reqres.in/img/faces/4-image.jpg")
        ...
        XCTAssertEqual(userResponse?.data[4].avatar,"https://reqres.in/img/faces/5-image.jpg", "The avatar should be https://reqres.in/img/faces/5-image.jpg")
        ...
        XCTAssertEqual(userResponse?.data[5].avatar,"https://reqres.in/img/faces/6-image.jpg", "The avatar should be https://reqres.in/img/faces/6-image.jpg")
        
        XCTAssertEqual(userResponse?.support.url, "https://reqres.in/#support-heading", "This should be https://reqres.in/#support-heading")
        XCTAssertEqual(userResponse?.support.text,"To keep ReqRes free, contributions towards server costs are appreciated!", "This should be To keep ReqRes free, contributions towards server costs are appreciated!")
    ...
    func test_with_invalid_file_error_thrown() {
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "sdgdsg", type: UsersResponse.self), "An error should be thrown")
        do {
            _ = try StaticJSONMapper.decode(file: "sgddgsd", type: UsersResponse.self)
        } catch {
            guard let mappingError = error as? StaticJSONMapper.MappingError else {
                XCTFail("This is the wrong type of error for missing files")
                return
